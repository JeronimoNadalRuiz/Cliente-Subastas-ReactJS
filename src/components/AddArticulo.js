...
class AddLote extends Component {
    ...
    componentDidMount() {
        ...
        this.loteId=pathArray[pathArray.length-1];
        //this.getLotes();
    }

    /*getLotes() {
        GetData('articulo/subasta/'+this.loteId, null, 'GET').then((res) => {
            if (res.status !== 'error') {
                console.log(res);
                this.setState({
                    lotes: res.message
    ...
    enviar = (e) => {
        ...
        e.preventDefault();
        this.changeState();
        PostData('articulo/create', this.state.articulo, 'POST').then((res) => {
            if(res.status!=='error'){
                succesMesage.html("<p>Artículo creado correctamente</p>").show();
                document.getElementById("addForm").reset(); 
                var btnSiguiente = $("#fin");
                btnSiguiente.prop('disabled', false);
                window.location.assign("/subasta");
                //this.getLotes();
            }else{
                alertMessage.html("<p>No puedes agregar artículos a subastas que no te pertenecen</p>").show();
                console.log(res);
            }
        });
    }
                    ...
                    <div className="row">
                        ...
                        <main className="col-md-10" id="page-content-wrapper">
                            ...
                            <div className="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pb-2 mt-3 mb-3 border-bottom">
                                <h1 className="h2">Añadir Artículo</h1>
                            </div>
